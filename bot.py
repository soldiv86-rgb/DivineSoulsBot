...
def get_icon_version() -> str:
    ...
    so even when it re-fetches manifest.json and sees the icon "changed",
    it can still reuse the old cached image bytes for that same
    "/icon-192.png" URL. Giving each distinct icon its own URL (via this
    version tag) forces a real re-fetch instead.
