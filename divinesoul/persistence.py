...
def _write_apks(data: dict) -> None:
    if _using_postgres():
        try:
            _pg_set("apks", data)
        except Exception as e:
            print(f"Failed to save apks to Neon: {e}", flush=True)
        return

    try:
        _write_json_atomic(APKS_META_FILE, data)
    except Exception as e:
        print(f"Failed to save apks to {APKS_META_FILE}: {e}", flush=True)
...
_apks_saver = _BackgroundSaver(lambda: copy.deepcopy(state.apks), _write_apks)
...
# ---- Uploaded APK/IPA builds (added/removed from the dashboard) ----
...
def load_apks() -> None:
    if _using_postgres():
        try:
            ...
            if isinstance(data, dict):
                state.apks = data
                print(f"Loaded {len(state.apks)} uploaded build(s) from Neon Postgres", flush=True)
            else:
                state.apks = {}
                if APKS_META_FILE.exists():
                    try:
                        with open(APKS_META_FILE, "r") as f:
                            state.apks = json.load(f)
                        if state.apks:
                            _write_apks(state.apks)
                            print(
                                f"Migrated {len(state.apks)} build(s) "
                                f"from {APKS_META_FILE} -> Neon",
                                flush=True,
                            )
                    except Exception as e:
                        print(f"Apks JSON migrate skipped ({e})", flush=True)
                        state.apks = {}
                else:
                    print("No uploaded builds in Neon yet - starting empty", flush=True)
        except Exception as e:
            print(f"Neon load_apks failed ({e}) - starting empty", flush=True)
            state.apks = {}
        return
    ...
    if APKS_META_FILE.exists():
        try:
            with open(APKS_META_FILE, "r") as f:
                state.apks = json.load(f)
            print(f"Loaded {len(state.apks)} uploaded build(s) from {APKS_META_FILE}", flush=True)
        except Exception as e:
            print(f"Could not read {APKS_META_FILE} ({e}) - starting empty", flush=True)
            state.apks = {}
    else:
        state.apks = {}
...
def save_apks() -> None:
    """Queue a background write of state.apks (does not block)."""
    _apks_saver.request()
