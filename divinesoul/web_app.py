...
async def handle_apk_list(request):
    """Tells the dashboard's APK tab which hardcoded builds exist, per
    platform, and whether each one's file is actually present on disk."""
    err = _check_dashboard_auth(request)
    if err:
        return err
    grouped = {"android": [], "ios": []}
    for entry in APKS:
        ...
        item = {"id": entry.get("id"), "name": entry.get("name", entry.get("id")), "available": available}
        if available:
...
async def handle_apk_download(request):
    """Serves one hardcoded build for direct browser download. Auth accepts
    the key as a query param (see _dashboard_key) since this is a plain
    link click/navigation, not a fetch call that can set a header."""
    err = _check_dashboard_auth(request)
    if err:
        return err
...
async def handle_apk_download_all(request):
    ...
    present = [
        ...
        and (platform is None or e.get("platform", "android") == platform)
    ]
    if not present:
