...
async def handle_script(request):
    """Roblox reporter script including the real secret (dashboard key required)."""
    if not hmac.compare_digest(request.query.get("key", ""), DASHBOARD_KEY):
        return web.json_response({"error": "unauthorized"}, status=401)

    base = PUBLIC_BASE_URL
    if not base:
        # Fall back to the address this page was loaded from (Render terminates
        # TLS in front of us, so trust X-Forwarded-Proto when it is present).
        proto = request.headers.get("X-Forwarded-Proto", request.scheme).split(",")[0].strip()
        base = f"{proto}://{request.host}"

    return web.json_response(
        {
            "script": build_roblox_script(base, REPORT_SECRET),
            "baseUrl": base,
            "publicBaseUrlSet": bool(PUBLIC_BASE_URL),
        },
