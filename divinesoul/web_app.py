...
from .config import (
    ...
    APKS,
    APK_UPLOAD_DIR,
    MAX_APK_UPLOAD_BYTES,
)
from . import state
from .persistence import save_accounts, save_theme, load_ui_settings, save_ui_settings, save_apks
from .theme import (
    HAS_PIL,
    resolve_theme,
...
def _seed_entries():
    """Hardcoded builds from config.APKS, tagged as not removable."""
    out = []
    for entry in APKS:
        out.append({
            "id": entry.get("id"),
...
def _uploaded_entries():
    """Builds added through the dashboard's Upload button, tagged removable."""
    out = []
    for apk_id, info in state.apks.items():
        out.append({
            "id": apk_id,
            "name": info.get("name", apk_id),
            "platform": info.get("platform", "android"),
            "file": APK_UPLOAD_DIR / info["filename"] if info.get("filename") else None,
            "removable": True,
        })
    return out
...
def _all_entries():
    return _seed_entries() + _uploaded_entries()


def _apk_by_id(apk_id):
    for entry in _all_entries():
        if entry.get("id") == apk_id:
            return entry
    return None


def _content_type_for(path):
    # iOS builds ship as .ipa, which isn't a real registered MIME type -
    # browsers just treat it as an opaque binary to save, which is what we
    # want. .apk gets the real Android package type.
    if path.suffix.lower() == ".ipa":
        return "application/octet-stream"
    return "application/vnd.android.package-archive"


async def handle_apk_list(request):
    """Tells the dashboard's APK tab which builds exist (hardcoded and
    uploaded), per platform, and whether each one's file is actually
    present on disk."""
    err = _check_dashboard_auth(request)
    if err:
        return err
    ...
    for entry in _all_entries():
        ...
        grouped.setdefault(entry["platform"], []).append(item)
    return web.json_response(grouped)


async def handle_apk_download(request):
    """Serves one build for direct browser download. Auth accepts the key
    as a query param (see _dashboard_key) since this is a plain link
    click/navigation, not a fetch call that can set a header."""
    err = _check_dashboard_auth(request)
    if err:
        return err
    entry = _apk_by_id(request.query.get("id", ""))
    if not entry:
        return web.json_response({"error": "unknown build id"}, status=404)
    path = entry.get("file")
    if not path or not path.is_file():
        return web.json_response({"error": "that build isn't on the server yet"}, status=404)
...
async def handle_apk_download_all(request):
    """Zips together every build that's currently present on disk and
    serves that as one download. Pass ?platform=android or ?platform=ios to
    scope it to just that platform; omit it to zip everything. Built in
    memory - fine for a handful of personal app builds, not huge files."""
    err = _check_dashboard_auth(request)
    if err:
        return err
    ...
    present = [
        ...
        if e.get("file") and e["file"].is_file()
        and (platform is None or e["platform"] == platform)
    ]
    if not present:
        return web.json_response({"error": "no builds are on the server yet"}, status=404)

    buf = io.BytesIO()
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        for entry in present:
            zf.write(entry["file"], arcname=entry["file"].name)
    buf.seek(0)
    filename = f"{platform}-builds.zip" if platform else "builds.zip"
    return web.Response(
        body=buf.getvalue(),
        headers={
...
_APK_EXT_BY_PLATFORM = {"android": ".apk", "ios": ".ipa"}


async def handle_apk_upload(request):
    """Accepts a build uploaded from the dashboard's Upload button
    (multipart form: key, platform, name (optional), file) and adds it to
    the uploaded-builds list. These can later be removed from the
    dashboard, unlike the hardcoded APKS list in config.py."""
    if not request.content_type or "multipart/form-data" not in request.content_type:
        return web.json_response({"error": "expected a multipart/form-data upload"}, status=400)
    ...
    while True:
        ...
        if part.name == "key":
            ...
            platform = (await part.text()).strip().lower()
        elif part.name == "name":
            ...
            while True:
                ...
                if not chunk:
                    break
                total += len(chunk)
                if total > MAX_APK_UPLOAD_BYTES:
                    return web.json_response({"error": "file is too large"}, status=413)
                chunks.append(chunk)
            file_bytes = b"".join(chunks)

    if _rate_limited(request):
        return web.json_response({"error": "too many attempts - try again in a minute"}, status=429)
    if not hmac.compare_digest(key, DASHBOARD_KEY):
        _record_failure(request)
        return web.json_response({"error": "unauthorized"}, status=401)

    if platform not in ("android", "ios"):
        return web.json_response({"error": "platform must be 'android' or 'ios'"}, status=400)
    if not filename or not file_bytes:
        return web.json_response({"error": "no file was uploaded"}, status=400)

    expected_ext = _APK_EXT_BY_PLATFORM[platform]
    orig_suffix = Path(filename).suffix.lower()
    if orig_suffix != expected_ext:
        return web.json_response(
            {"error": f"expected a {expected_ext} file for {platform}, got {orig_suffix or 'no extension'}"},
            status=400,
        )

    APK_UPLOAD_DIR.mkdir(parents=True, exist_ok=True)
    apk_id = uuid.uuid4().hex[:12]
    stored_name = f"{apk_id}{expected_ext}"
    try:
        (APK_UPLOAD_DIR / stored_name).write_bytes(file_bytes)
    except Exception as e:
        return web.json_response({"error": f"could not save file: {e}"}, status=500)
    ...
    save_apks()
    return web.json_response({"ok": True, "id": apk_id})


async def handle_apk_remove(request):
    """Removes a build that was added through the Upload button. Hardcoded
    entries from config.APKS can't be removed this way - edit that file
    instead - and this returns an error if asked to."""
    try:
        data = await request.json()
    except Exception:
        return web.json_response({"error": "invalid json"}, status=400)
    ...
    apk_id = str(data.get("id", ""))
    if apk_id not in state.apks:
        return web.json_response(
            {"error": "not found, or it's a hardcoded build that can only be removed by editing config.py"},
            status=404,
        )

    info = state.apks.pop(apk_id)
    save_apks()
    filename = info.get("filename")
    if filename:
        try:
            (APK_UPLOAD_DIR / filename).unlink(missing_ok=True)
        except Exception as e:
            print(f"Could not delete uploaded file {filename}: {e}", flush=True)
...
async def start_web_server():
    # aiohttp's default client_max_size (~a couple MB) would reject any real
    # APK/IPA upload before handle_apk_upload even runs, so raise it to fit
    # MAX_APK_UPLOAD_BYTES plus some headroom for the other form fields.
    app = web.Application(client_max_size=MAX_APK_UPLOAD_BYTES + 1024 * 1024)
    ...
    app.router.add_get("/apk/list", handle_apk_list)
    app.router.add_get("/apk/download", handle_apk_download)
    app.router.add_get("/apk/download-all", handle_apk_download_all)
    app.router.add_post("/apk/upload", handle_apk_upload)
    app.router.add_post("/apk/remove", handle_apk_remove)
