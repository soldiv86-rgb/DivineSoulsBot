...
from . import state
from .persistence import save_accounts, save_theme, _pg_get, _pg_set, _using_postgres
from .theme import (
    HAS_PIL,
    resolve_theme,
...
DEFAULT_UI_SETTINGS = {
    "pinned": [],
    "sort": "online_first",  # online_first | name | last_seen
    ...
    try:
        if _using_postgres():
            ...
            if isinstance(data, dict):
                out = {**DEFAULT_UI_SETTINGS, **data}
                out["quietHours"] = {**DEFAULT_UI_SETTINGS["quietHours"], **(data.get("quietHours") or {})}
                return out
        else:
            import json
        ...
        print(f"load_ui_settings: {e}", flush=True)
